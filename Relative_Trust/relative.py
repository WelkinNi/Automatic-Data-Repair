...
class RelaTrust:
    ...
    def evaluation(self):
        ...
        if True:
            if not PERFECTED:
                det_right = 0
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/Relative_trust/" + task_name[:-1] +"/onlyED_" + task_name + dirty_path[-25:-4] + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
                ...
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/Relative_trust/" + task_name[:-1] +"/oriED+EC_" + task_name + dirty_path[-25:-4] + ".txt"
                res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/Relative_trust/" + task_name[:-1] + "/repaired_" + task_name + dirty_path[-25:-4] + ".csv"
                ...
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/Relative_trust/" + task_name[:-1] +"/prefectED+EC_" + task_name + dirty_path[-25:-4] + ".txt"
                res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/Relative_trust/" + task_name[:-1] + "/perfect_repaired_" + task_name + dirty_path[-25:-4] + ".csv"
                self.rep_csv.to_csv(res_path, index=False, columns=list(self.rep_csv.columns))
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
