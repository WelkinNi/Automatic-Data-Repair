...
class Holistic():
    ...
    def run(self, file_path, dirty_path, clean_path):
        ...
        if True:
            ...
            if not PERFECTED:
                det_right = 0
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/holistic/" + task_name[:-1] +"/onlyED_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
                ...
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/holistic/" + task_name[:-1] +"/oriED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/holistic/" + task_name[:-1] +"/repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
                ...
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/holistic/" + task_name[:-1] +"/all_computed_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                right2wrong = 0
                right2right = 0
                ...
                rec = rec_right / (wrong_cells+1e-10)
                f1 = 2*pre*rec / (rec+pre+1e-10)
                out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/holistic/" + task_name[:-1] +"/perfectED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/holistic/" + task_name[:-1] +"/perfect_repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
                dirty_df = pd.read_csv(dirty_path)
                for cell, value in self.repaired_cells_value.items():
                    dirty_df.iloc[cell[0], cell[1]] = value
                dirty_df.to_csv(res_path, index=False)
...
if __name__ == "__main__":
    ...
    # dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/flights/noise/flights-inner_error-10.csv"
    # dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/hospital/dirty.csv"
    ...
    try:
        ...
        out_file.write("Timeout: Holistic.py: ")
