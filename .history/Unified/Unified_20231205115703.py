...
class Unified():
    def __init__(self, csv_d, csv_c, fd_path, theta=0.01, beta=0.4):
        ...
        self.out_csv = "./Repaired_res/Unified/" + task_name[:-1] + "/repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
    ...
    def evaluation(self):
        ...
        if True:
            if not PERFECTED:
                ...
                det_right = 0
                out_path = "./Exp_result/Unified/" + task_name[:-1] +"/onlyED_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
                ...
                out_path = "./Exp_result/Unified/" + task_name[:-1] +"/oriED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                res_path = "./Repaired_res/Unified/" + task_name[:-1] + "/repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
                ...
                out_path = "./Exp_result/Unified/" + task_name[:-1] + "/all_compute_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                f = open(out_path, 'w')
                sys.stdout = f
                right2wrong = 0
                right2right = 0
                ...
                out_path = "./Exp_result/Unified/" + task_name[:-1] +"/prefectED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
                res_path = "./Repaired_res/Unified/" + task_name[:-1] + "/perfect_repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
                self.dirty_csv.to_csv(res_path, index=False)
                f = open(out_path, 'w')
                sys.stdout = f
                end_time = time.time()
...
if __name__ == "__main__":
    ...
    # task_name = "flights1"
    # out_path = "./Exp_result/Unified/" + task_name[:-1] +"/onlyED_" + task_name + check_string(dirty_path.split("/")[-1]) + dirty_path[-6:-4] + ".txt"
    # ONLYED = 1
    # PERFECTED = 0
