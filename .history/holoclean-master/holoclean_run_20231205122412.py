...
if __name__ == "__main__": 
    ...
    # if ONLYED:
    #     out_path = "./Exp_result/holoclean/" + task_name[:-1] +"/onlyED_" + task_name + dirty_path[-25:-4] + ".txt"
    #     f = open(out_path, 'w')
    #     sys.stdout = f
    #     end_time = time.time()
    ...
    if not PERFECTED:
        out_path = "./Exp_result/holoclean/" + task_name[:-1] + "/onlyED_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
        f = open(out_path, 'w')
        sys.stdout = f
        end_time = time.time()
        print("{pre}\n{rec}\n{f1}\n{time}".format(pre=det_pre, rec=det_rec, f1=det_f1, time=(end_time-start_time)))
        ...
        out_path = "./Exp_result/holoclean/" + task_name[:-1] + "/all_compute_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
        f = open(out_path, 'w')
        sys.stdout = f
        right2wrong = 0
        right2right = 0
        ...
        out_path = "./Exp_result/holoclean/" + task_name[:-1] + "/oriED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
        res_path = "./Repaired_res/holoclean/" + task_name[:-1] + "/repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
        ...
        out_path = "./Exp_result/holoclean/" + task_name[:-1] + "/perfectED+EC_" + task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
        res_path = "./Repaired_res/holoclean/" + task_name[:-1] + "/perfect_repaired_" + task_name + check_string(dirty_path.split("/")[-1]) + ".csv"
        repaired_df.to_csv(res_path, index=False, columns=list(repaired_df.columns))
        f = open(out_path, 'w')
        sys.stdout = f
        end_time = time.time()
