...
def process_method(method_name, task_name, i, j, error_type, detection_dictionary):
    ...
    clean_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/clean.csv"
    dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/noise/" + task_name + error_type + str(j) + ".csv"
    method_res_dir = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/" + method_name + "/" + task_name + "/repaired_" + task_name + str(i) + error_type + str(j) + ".csv"

    try:
        clean_df = pd.read_csv(clean_path).astype(str).fillna("nan")
        dirty_df = pd.read_csv(dirty_path).astype(str).fillna("nan")  
        method_df = pd.read_csv(method_res_dir).astype(str).fillna("nan")
        ori_error_num = 0
        rep_error_num = 0
        error_cell_list = detection_dictionary.keys()
        
        for r in range(len(clean_df)):
            for c in range(len(clean_df.columns)):
                if (r, c) not in error_cell_list:
                    method_df.iloc[r, c] = dirty_df.iloc[r, c]
        improve_res_dir = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/" + method_name + "/" + task_name + "/Raha_improved-" + task_name + str(i) + error_type + str(j) + ".csv"
        method_df.to_csv(improve_res_dir, index=False)
        print("Finished: " + improve_res_dir)
    except FileNotFoundError as e:
        logging.error("File not found: %s", e)
        return (method_name, task_name, i, j, error_type, "nan")
    except Exception as e:
        logging.error("An error occurred: %s %s", e, method_res_dir)
        return (method_name, task_name, i, j, error_type, "nan")
                
    return (method_name, task_name, i, j, error_type, float((ori_error_num-rep_error_num)/ori_error_num))
########################################
def process_boostclean(method_name, task_name, i, j, error_type, detection_dictionary):
    
    clean_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/clean.csv"
    dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/noise/" + task_name + error_type + str(j) + ".csv"
    # dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/noise/" + task_name + '-dirty-original_error-' + str(j) + ".csv"
    method_res_dir = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/" + method_name + "/" + task_name + "/repaired_" + task_name + str(i) + error_type + str(j) + ".csv"
    ...
    try:
        ...
        for r in range(len(clean_df)):
            if not flag:
                rep_idx = r
                if dirty_df.iloc[r, 1] != method_df.iloc[r, 1]:
                    flag = 1
                    rep_idx = r 
                    continue
            for c in range(len(clean_df.columns)):
                if (r, c) not in error_cell_list:
                    method_df.iloc[rep_idx, c] = dirty_df.iloc[r, c]
...
if __name__ == "__main__":
    ...
    for i in range(1,4):
        for task_name in task_names:
            pool = multiprocessing.Pool(processes=8)  # 不能太大了，否则内存会炸
            for error_type in error_types:
                for j in error_rate:
                    dataset_name = task_name + error_type + str(i) + "-" + str(j)

                    dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/noise/" + task_name + error_type + str(j) + ".csv"
                    # dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/split_data/" + task_name + '-dirty-original_error-' + str(j) + ".csv"
                    
                    clean_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/clean.csv"
                    # clean_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/" + task_name + "/split_data/tax-clean-clean_data_ori-0010k.csv"
                    dataset_dictionary = {
                        "name": dataset_name,
                        "path": dirty_path,
                    ...
                    for method_name in method_names:
                        args = (method_name, task_name, i, j, error_type, detection_dictionary)
                        if method_name != 'boostclean':
                            results.append(pool.apply_async(process_method, args))
                        else:
                            results.append(pool.apply_async(process_boostclean, args))
