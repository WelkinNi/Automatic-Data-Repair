...
import numpy as np
import argparse
sys.path.append('/data/nw/DC_ED/References_inner_and_outer/activedetect-master/')
...
if __name__ == "__main__":
    ...
    # dirty_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/data with dc_rules/hospital/noise/hospital-outer_error-90.csv"
    ...
    if True:
        if not PERFECTED:
            out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/boostclean/" + task_name[:-1] + "/onlyED_" + task_name + check_string(dirty_path) + ".txt"
            f = open(out_path, 'w')
            sys.stdout = f
            end_time = time.time()
            ...
            out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/boostclean/" + task_name[:-1] + "/oriED+EC_" + task_name + check_string(dirty_path) + ".txt"
            res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/boostclean/" + task_name[:-1] + "/repaired_" + task_name + check_string(dirty_path) + ".csv"
            ...
            out_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Exp_result/boostclean/" + task_name[:-1] + "/prefectED+EC_" + task_name + check_string(dirty_path) + ".txt"
            res_path = "/data/nw/DC_ED/References_inner_and_outer/DATASET/Repaired_res/boostclean/" + task_name[:-1] + "/perfect_repaired_" + task_name + check_string(dirty_path) + ".csv"
            for res in rep_result:
                if len(res) == 11:
                    res.remove(res[-3])
